...
class CadEstoque(CTkToplevel):
    ...
    def carreagar_widgets(self):
        ...
        CTkLabel(self, text='Codigo de Barra', font=self.font_label).grid(column=0, row=0, padx=10, sticky='w')
        self.cod_barra.grid(column=0, row=1, padx=10, sticky='w')
        CTkLabel(self, text='Descrição', font=self.font_label).grid(column=0, row=2, padx=10, sticky='w')
        self.descricao.grid(column=0, row=3, padx=10, sticky='w')
        CTkLabel(self, text='Preço Unitário', font=self.font_label).grid(column=0, row=4, padx=10, sticky='w')
        self.preco_uni.grid(column=0, row=5, padx=10, sticky='w')
        CTkLabel(self, text='Quantidade Mínima', font=self.font_label).grid(column=0, row=6, padx=10, sticky='w')
        self.quantidade_min.grid(column=0, row=7, padx=10, sticky='w')
        CTkLabel(self, text='Quantidade Atual', font=self.font_label).grid(column=0, row=8, padx=10, sticky='w')
        self.quantidade_atual.grid(column=0, row=9, padx=10, sticky='w')
        CTkLabel(self, text='Quantidade Máxima', font=self.font_label).grid(column=0, row=10, padx=10, sticky='w')
        self.quantidade_max.grid(column=0, row=11, padx=10, sticky='w')
        CTkLabel(self, text='Fornecedor', font=self.font_label).grid(column=0, row=12, padx=10, sticky='w')
        self.fornecedor.grid(column=0, row=13, padx=10, sticky='w')
    
        CTkButton(self, text='Cadastrar', font=self.font_button, height=40, compound='left', image=CTkImage(Image.open(img_cadastrar))).grid(column=0, row=14)
